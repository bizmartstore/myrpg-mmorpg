...
// Calculate player stats based on class & level
...
// Broadcast event to players in AOI
    ...
    if (closestPlayer && closestDist < monster.aggroRange) {
      ...
      if (closestDist > monster.attackRange) {
        ...
        if (now - monster.lastAttack > monster.attackCooldown) {
          ...
          targetPlayer.hp = Math.max(0, targetPlayer.hp - monster.attack);
          ...
          if (targetPlayer.hp === 0) handlePlayerDeath(targetPlayer);
...
function handlePlayerDeath(player) {
  ...
  if (mapPlayers.has(player.map)) mapPlayers.get(player.map).delete(player.email);
  ...
  io.to(player.socketId).emit('player:died', { map: townMap, x: player.x, y: player.y });
  ...
  setTimeout(() => {
    ...
    io.to(player.socketId).emit('player:revived', { hp: player.hp, maxHp: player.maxHp });
  }, 3000);
}
...
io.on('connection', (socket) => {
  ...
  socket.on('player:join', (data) => {
    ...
    currentPlayer = {
      email, name, character_class, level,
      x: position.x, y: position.y,
      direction: 'front', state: 'idle', map,
      socketId: socket.id, lastUpdate: Date.now(),
      hp: stats.maxHp, maxHp: stats.maxHp, attack: stats.attack, isDead: false,
      xp: 0, inventory: []
    };
    ...
    // Send existing monsters to player
    ...
    for (const monsterId of monstersInMap) {
      ...
      if (m && m.hp > 0) {
        socket.emit('monster:spawn', {
          id: m.id, type: m.type, mapId: m.mapId, x: m.x, y: m.y,
          hp: m.hp, maxHp: m.maxHp, direction: m.direction, state: m.state,
          spawnX: m.spawnX, spawnY: m.spawnY, target: m.target
        });
      }
    }
  ...
  socket.on('player:attack', (data) => {
    ...
    broadcastToAOI(currentPlayer.email, currentPlayer.x, currentPlayer.y, currentPlayer.map, 'player:attacked', {
  ...
  socket.on('player:skill', (data) => {
    ...
    broadcastToAOI(currentPlayer.email, currentPlayer.x, currentPlayer.y, currentPlayer.map, 'player:skill', {
      email: currentPlayer.email,
      skillType,
      position: { x: currentPlayer.x, y: currentPlayer.y },
      direction: currentPlayer.direction,
  ...
  socket.on('monster:hit', (data) => {
    ...
    broadcastToMap(monster.mapId, 'monster:hit', { id: monsterId, mapId: monster.mapId, hp: monster.hp, damage });
    ...
    setTimeout(() => {
      ...
      broadcastToMap(monster.mapId, 'monster:spawn', {
        id: monster.id, type: monster.type, mapId: monster.mapId,
        x: monster.x, y: monster.y, hp: monster.hp, maxHp: monster.maxHp,
        direction: monster.direction, state: monster.state
      });
    }, 5000);
  });
